angular.module('pliApp').controller('homeController', function($scope, $location, $http, $window, userFactory) {
  ...
  $scope.ConnectionOneDrive = function(){
    console.log("click");
     FindOnedrive = false;
      for(var i = 0; i < tabCloud.length; i++){
        if (tabCloud[i].id_cloud === 3){
          FindOnedrive = true;
        }
      }

      if(FindOnedrive == false)
      {
        console.log("inside connexion onedrive");
        
        $http({
          url: "/api/onedrive/url", method: 'POST',
          data: $.param({ 'userId':userName.userId, 'idCloud' : 3 }),
          headers : {'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'}
        }).then(function(response) {
          if(response.status === 200) {
            console.log(response.status);
